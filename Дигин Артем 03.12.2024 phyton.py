...
choise =input(f'Куда пойдешь?[север,юг,запад,восток]')
# север ключ .юг карта .запад супер приз автомобиль.восток ничего
if choise == 'север':
    print('холод истратили большой запас сил')
    energy -= 30
    has_key = True
    print(f'Текущий запас энергии:{energy}')
    print('На пути попался медведь...,пришлось бежать')
elif choise == 'юг':
    print('Вы нашли карту,на юге загорели')
    energy += 20
    has_map = True
    print(f'Текущий запас энергии:{energy}')
    print('Вы нашли карту')
elif choise == 'запад':
    print('Вы нашли автомобиль,ну без ключа его не открыть')

elif choise == 'восток':
    print('На востоке ничего')
    energy -= 20
    print(f'Текущий запас энергии:{energy}')
